...
# ── OLD: Binary-search based _optimize_gpu_layers ──
# Replaced by projection-based version below.
# Kept for reference — delete after validation.
#
...
async def _optimize_gpu_layers(
    ...
    """Projection-based layer optimization for fast GPU group.

    Maximizes layers on the fastest GPU group at a fixed context size.
    Uses fit-params projection + real VRAM measurement instead of binary search.

    Algorithm:
    1. Fit-params projection → estimate max fast_total from projected free VRAM
    2. Physical test at estimated split → get real per-GPU free VRAM
    3. Per-GPU greedy shift: move layers from slow→fast based on real VRAM data
    4. Confirmation test if shifts were applied

    Typically 2-3 physical tests instead of 5-8 with binary search.
    ...
    initial_fast_total = sum(initial_layers[i] for i in fast_indices)
    padded_ratios = _build_balanced_split(total_layers, initial_fast_total, active_gpu_names)
    padded_ratios += [0.0] * (total_gpus - min_gpus)
    initial_cmd = _replace_tensor_split(base_cmd, padded_ratios)

    # ── Step 1: Fit-params projection ──
    # Measure VRAM occupied by other processes (TTS, CUDA driver, etc.).
    # fit-params uses total physical VRAM — it doesn't know about TTS.
    # Subtracting occupied VRAM gives accurate projections even with TTS loaded.
    vram_occupied: list[int] = []
    baseline_gpu = get_all_gpus_memory_info()
    if baseline_gpu and baseline_gpu["per_gpu"]:
        baseline_sorted = _to_cuda_order(
            [dict(g) for g in baseline_gpu["per_gpu"]]
        )
        vram_occupied = [g["total_mb"] - g["free_mb"] for g in baseline_sorted]
    ...
    # Correct fit-params projections for external VRAM usage (TTS, etc.)
    if vram_occupied:
        for gid in gpu_ids:
            idx = int(gid.replace("CUDA", ""))
            if idx < len(vram_occupied) and gid in per_gpu:
                per_gpu[gid]["free"] -= vram_occupied[idx]
    ...
    def _optimal_fill(
        ...
        """Calculate per-GPU optimal layer distribution.

        Each fast GPU gets as many layers as its free VRAM allows.
        Remaining layers go to slow GPUs (balanced within slow group).
        """
        target = [0.0] * min_gpus
        remaining = total_layers

        # Fast GPUs: fill each to its individual max
        for fi in fast_indices:
            headroom = gpu_free.get(fi, 0)
            extra = max(0, int(headroom / mb_per_layer))
            take = min(base_layers[fi] + extra, remaining)
            target[fi] = float(take)
            remaining -= int(target[fi])

        # Slow GPUs: distribute remaining (balanced within slow group)
        slow_indices = [i for i in range(min_gpus) if i not in fast_set]
        if remaining > 0 and slow_indices:
            per_slow = remaining // len(slow_indices)
            rem = remaining % len(slow_indices)
            for j, si in enumerate(slow_indices):
                target[si] = float(per_slow + (1 if j < rem else 0))

        return target

    # ── Step 2: Per-GPU optimal fill from fit-params projection ──
    # Each GPU gets layers based on its individual free VRAM.
    # No balanced distribution — CUDA1 can get more than CUDA0
    # (CUDA0 has embedding/LM-head overhead eating into its VRAM).
    proj_free = {
        i: per_gpu.get(f"CUDA{i}", {}).get("free", 0)
        for i in range(min_gpus)
    }
    est_target = _optimal_fill(initial_layers, proj_free)
    est_padded = est_target + [0.0] * (total_gpus - min_gpus)
    est_split = _format_layer_split(est_padded)

    if est_padded == padded_ratios:
        yield f"No headroom for layer optimization on {fastest_name} (x{fast_count})"
        yield {"tested": False}
        yield padded_ratios
        return
    ...
    yield (
        f"Phase {phase_label}: Optimizing per-GPU at "
        f"ctx={format_number(target_context)} — target {est_split}"
    )

    # ── Step 2→N: Iterative test → recalculate from real VRAM ──
    # 1. Test projected split
    # 2. Fits → recalculate from real nvidia-smi data → more room? → test again
    # 3. OOM → halve headroom, recalculate → test again
    # Converges: success shrinks headroom, OOM halves it.
    iteration = 0
    best_ratios = list(padded_ratios)
    best_gpus: list[dict[str, Any]] | None = None
    current = est_padded

    while current != list(best_ratios):
        iteration += 1
        cur_split = _format_layer_split(current)
        cur_cmd = _replace_tensor_split(base_cmd, current)

        fits, _, gpus, _ = await _test_context_physical(
            cur_cmd, target_context, port, safety_margin=safety_margin,
        )
        yield _fmt_test(
            f"{phase_label}.{iteration}", target_context,
            fits=fits, gpu_list=gpus, split=cur_split,
        )

        if fits:
            best_ratios = [float(r) for r in current]
            best_gpus = gpus
            # Recalculate from real VRAM: any GPU still has room?
            real_free = {
                i: gpus[i].get("free_mb", 0) - safety_margin
                for i in range(min_gpus) if i < len(gpus)
            }
            current_layers = [int(r) for r in best_ratios[:min_gpus]]
            recalc = _optimal_fill(current_layers, real_free)
            current = recalc + [0.0] * (total_gpus - min_gpus)
            # If no change → loop exits via != check
        else:
            # OOM — halve the delta between current attempt and last known good
            delta_total = sum(
                current[i] - best_ratios[i]
                for i in range(min_gpus)
                if current[i] > best_ratios[i]
            )
            if delta_total <= 1:
                break  # Can't halve a single layer
            # Halve the headroom that led to this attempt
            halved_free = {
                i: max(0, (current[i] - best_ratios[i]) * mb_per_layer / 2)
                for i in range(min_gpus)
            }
            base_layers = [int(r) for r in best_ratios[:min_gpus]]
            recalc = _optimal_fill(base_layers, halved_free)
            current = recalc + [0.0] * (total_gpus - min_gpus)

    padded_ratios = [float(r) for r in best_ratios]
    best_split = _format_layer_split(padded_ratios)
    tested = best_gpus is not None
    yield f"Phase {phase_label} result: {best_split} ({min_gpus}/{total_gpus} GPUs)"
    yield {"tested": tested}
    yield padded_ratios
...
async def _try_speed_split_for_gpu_count(
    ...
    # ── Phase A: Optimize layers on fast GPU group ──
    phase_a_tested = False
    async for opt_result in _optimize_gpu_layers(
        ...
        if isinstance(opt_result, dict):
            phase_a_tested = opt_result.get("tested", False)
        elif isinstance(opt_result, list):
            padded_ratios = opt_result
        else:
            yield opt_result

    if not phase_a_tested:
        yield f"Speed: {min_gpus} GPUs — no working split found"
        yield "__SPEED__:0"
        return
            ...
            if total_layers_1b:
                ...
                if min_gpus_1b < total_gpus_1b:
                    ...
                    for n_gpus in range(min_gpus_1b, total_gpus_1b):
                        ...
                        # Initial balanced split for this GPU count
                        groups_1b = _group_gpus_by_name(active_names_1b)
                        active_vram = per_gpu_total_mb_sorted[:n_gpus]
                        fast_vram = sum(active_vram[i] for i in groups_1b[0])
                        total_active_vram = sum(active_vram)
                        initial_fast = min(
                            round(total_layers_1b * fast_vram / total_active_vram),
                            total_layers_1b,
                        )
                        layers_1b = [
                            int(r) for r in _build_balanced_split(
                                total_layers_1b, initial_fast, active_names_1b,
                            )
                        ]

                        yield (
                            f"Phase 1b: trying {n_gpus} GPUs "
                            f"at ctx={format_number(best_result)}"
                        )

                        # Let _optimize_gpu_layers find the right split
                        # (projection-based — no naive test needed)
                        ...
                        async for opt_result in _optimize_gpu_layers(
                            ...
                            if isinstance(opt_result, dict):
                                opt_found = opt_result.get("tested", False)
                            elif isinstance(opt_result, list):
                                optimized_ratios = opt_result
                            else:
                                yield opt_result

                        if opt_found and optimized_ratios:
                            ...
                            yield (
                                f"Phase 1b: {n_gpus} GPUs insufficient "
                                f"at ctx={format_number(best_result)}"
                            )
