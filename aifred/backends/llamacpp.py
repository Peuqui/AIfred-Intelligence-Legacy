...
llama.cpp provides a high-performance C++ inference engine for LLMs.
The llama-server (previously llama-cpp-python server) provides an OpenAI-compatible API.
"""
...
class LlamaCppBackend(LLMBackend):
    """llama.cpp backend implementation (via llama-server with OpenAI-compatible API)"""

    def __init__(self, base_url: str = "http://localhost:8080/v1", api_key: str = "dummy"):
        super().__init__(base_url=base_url, api_key=api_key)
        self.client = AsyncOpenAI(
            base_url=base_url,
            api_key=api_key,  # llama.cpp server doesn't need real API key
            timeout=120.0  # llama.cpp may need time for context processing
        )

    async def list_models(self) -> List[str]:
        """Get list of available models from llama.cpp"""
        try:
    ...
    async def chat(
        ...
        """
        Non-streaming chat with llama.cpp

        Args:
            model: Model name (usually the loaded model)
    ...
    async def preload_model(self, model: str) -> tuple[bool, float]:
        ...
        Note: llama.cpp server typically loads one model at startup,
        so this method mainly serves to warm up the context/cache.
        ...
        try:
            ...
            # Send minimal request to warm up (OpenAI-compatible API)
    ...
    async def get_backend_info(self) -> Dict:
        """Get llama.cpp backend information"""
        try:
            ...
            return {
                ...
                "models": models,
                "healthy": True,
                "api_type": "OpenAI-compatible (llama-server)"
            }
        except Exception as e:
            return {
                "backend": "llama.cpp",
    ...
    async def get_model_context_limit(self, model: str) -> int:
        ...
        Queries /v1/models endpoint and extracts context window info.
        ...
        """
        try:
            # llama.cpp follows OpenAI-compatible /v1/models/{model} endpoint
            import httpx
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                # Check for n_ctx (llama.cpp specific)
                if "n_ctx" in data:
                    return int(data["n_ctx"])

                # Fallback: check in metadata
                if "metadata" in data:
                    if "n_ctx" in data["metadata"]:
                        return int(data["metadata"]["n_ctx"])
                ...
                # Default fallback for llama.cpp (most models support at least 2048)
                logger.warning(f"Context limit not found for llama.cpp model '{model}', using default 2048")
                return 2048

        except Exception as e:
            logger.warning(f"Failed to query llama.cpp for model '{model}': {e}, using default 2048")
