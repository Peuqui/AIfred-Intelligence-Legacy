...
// ============================================================
// TTS AUTO-PLAY HELPER
// ============================================================

// Track last played audio to prevent double-play
let lastPlayedAudioSrc = '';
let audioObserver = null;

// Function to play TTS audio
function playTtsAudio(audioSrc) {
    console.log('🔊 playTtsAudio called with:', audioSrc);

    if (!audioSrc || audioSrc === lastPlayedAudioSrc) {
        console.log('⏭️ Skipping - same audio or empty src');
        return;
    }

    const audioPlayer = document.getElementById('tts-player');
    if (audioPlayer) {
        console.log('🎵 Found audio player, attempting playback...');
...
// Observer to detect when audio element gets autoPlay attribute
function setupAudioObserver() {
    // Watch for changes to audio elements
    ...
    audioObserver = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            // Check for new audio elements with autoplay
            if (mutation.type === 'childList') {
                mutation.addedNodes.forEach((node) => {
                    if (node.nodeType === 1) {  // Element node
                        const audioElements = node.tagName === 'AUDIO' ? [node] :
                                             node.querySelectorAll ? node.querySelectorAll('audio[autoplay]') : [];

                        audioElements.forEach((audio) => {
                            if (audio.hasAttribute('autoplay') && audio.src) {
                                console.log('🔊 New audio with autoplay detected:', audio.src);
                                // Small delay to let the element fully initialize
                                setTimeout(() => {
                                    audio.play().then(() => {
                                        lastPlayedAudioSrc = audio.src;
                                        console.log('✅ Auto-triggered playback');
                                    }).catch(err => {
                                        console.warn('⚠️ Autoplay blocked:', err.message);
                                    });
                                }, 100);
    ...
    console.log('🔊 Audio observer set up');
}

// Setup audio observer after DOM ready
setTimeout(setupAudioObserver, 500);
