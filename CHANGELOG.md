...
- **Metadata line breaks** - All spaces within metric values replaced with non-breaking spaces; only break point is between Inference and Source, preventing mid-value wrapping on any browser
- **Metadata font size** - Reduced to 0.9em via `p:last-child > em:only-child` selector (targets only metadata, not regular italic text)
- **Session title generation** - Added `finally` block to catch silent generator cancellations (`GeneratorExit`/`CancelledError`); previously large models (120B+) would show "Generating session title..." with no follow-up
- **Mobile button layout** - Recording button and upload buttons now fill full width on mobile (flex 2:1:1:1), recording button consistently largest
